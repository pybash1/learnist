...
export default function Schedule({ classes }) {
  ...
  const timeline = classes?.map((class_, ind) => (
    <li className="mb-10 ml-4" key={ind}>
      ...
      <h3
        className="text-lg font-semibold text-gray-900 dark:text-white cursor-pointer hover:text-accent transition ease-in-out"
        onClick={() => {
          setName(class_?.name);
          setTeacher(class_?.teacher);
          setStime(format(utcToZonedTime(class_?.stime, timezone), "HH:mm"));
          setEtime(format(utcToZonedTime(class_?.etime, timezone), "HH:mm"));
        }}
      >
        {class_?.name}
      </h3>
  ...
  return (
    <div className="grid grid-cols-2 gap-8">
      ...
      <div className="bg-neutral rounded-[30px] p-10 text-white">
        ...
        <div className="flex items-center">
          <h2 className="text-xl font-semibold">Taught by</h2>
          &nbsp;
          <strong className="font-normal">{teacher}</strong>
        </div>
        <div className="flex items-center">
          <h2 className="text-xl font-semibold">Starts at</h2>
          &nbsp;
          <strong className="font-normal">{stime}</strong>
        </div>
        <div className="flex items-center">
          <h2 className="text-xl font-semibold">Ends at</h2>
          &nbsp;
          <strong className="font-normal">{etime}</strong>
        </div>
      </div>
    </div>
  );
